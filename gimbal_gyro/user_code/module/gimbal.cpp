...
void Gimbal::init()
{
    ...
    gimbal_INT_gyro_point = get_IMU_gyro_point();
    ...
    //读取我方颜色，初始为自瞄模式
    //vision_mode_change_color_flag = can_receive.gimbal_receive.color;
    vision_mode_change_shoot_flag = AUTO;
    press_vision_shoot_change_flag = 0;
    press_vision_color_change_flag = 0;
    //改为识别对方颜色
    if(vision_mode_change_color_flag == RED)
    {
        vision_mode_change_color_flag = BLUE;
    }
    else
    {
        vision_mode_change_color_flag = RED;
    }
...
{
    /*------------------------yaw电机数据更新----------------------------  */
    gimbal_yaw_motor.gyro_angle = gimbal_INT_angle_point[0];
    #if YAW_TURN
...
//根据电机正反转,调解电流发送
#if YAW_TURN
    gimbal_yaw_motor.current_set = - gimbal_yaw_motor.current_set;
#else
    gimbal_yaw_motor.current_set = gimbal_yaw_motor.current_set;
#endif
...
#if PITCH_TURN
    gimbal_pitch_motor.current_set = - gimbal_pitch_motor.current_set;
#else
    gimbal_pitch_motor.current_set = gimbal_pitch_motor.current_set;
#endif
    ...
    if (gimbal_mode == GIMBAL_ZERO_FORCE)
    {
        gimbal_yaw_motor.current_set = 0;
        gimbal_pitch_motor.current_set = 0;
    }

    can_receive.can_cmd_torque_control(CAN_YAW_GIMOTOR_ID,gimbal_yaw_motor.current_set,20);
    can_receive.can_cmd_torque_control(CAN_PITCH_GIMOTOR_ID,gimbal_pitch_motor.current_set,20);
}
#endif
...
void Gimbal::key_state_update()
{
    ...
    if (press_stop_time == PRESS_STOP_LONG_TIME && gimbal_stop_flag == false) //长按自锁按键并且关闭完毕
    {
       ...
       press_vision_color_change = if_key_pessed(gimbal_RC->key.v, KEY_PRESSED_VISION_COLOR_CHANGE);
    press_vision_shoot_change = if_key_pessed(gimbal_RC->key.v, KEY_PRESSED_VISION_SHOOT_CHANGE);

    //识别装甲板颜色模式切换
    if(press_vision_color_change >= TRUE && press_vision_color_change_flag == 0)
    {
        if(vision_mode_change_color_flag == RED)
        {
            vision_mode_change_color_flag = BLUE;
        }
        else if(vision_mode_change_color_flag == BLUE)
        {
            vision_mode_change_color_flag = RED;
        }
        press_vision_color_change_flag = 1;
    }
    else if(press_vision_color_change == FALSE)
    {
        press_vision_color_change_flag = 0;
    }
    //自瞄和打符模式切换判断
    if(press_vision_shoot_change >= TRUE && press_vision_shoot_change_flag == 0)
    {
        if(vision_mode_change_shoot_flag == AUTO)
        {
            vision_mode_change_shoot_flag = BUFF;
        }
        else if(vision_mode_change_shoot_flag == BUFF)
        {
            vision_mode_change_shoot_flag = AUTO;
        }
        press_vision_shoot_change_flag = 1;
    }
    else if(press_vision_shoot_change == FALSE)
    {
        press_vision_shoot_change_flag = 0;
    }
...
void Gimbal::set_mode()
{
    ...
    //设置视觉发送模式
    if(vision_mode_change_color_flag == RED && vision_mode_change_shoot_flag == AUTO)
    {
        vision_cmdid = 0x00;
    }
    else if(vision_mode_change_color_flag == BLUE && vision_mode_change_shoot_flag == AUTO)
    {
        vision_cmdid = 0x01;
    }
    else if(vision_mode_change_color_flag == RED && vision_mode_change_shoot_flag == BUFF)
    {
        vision_cmdid = 0x02;
    }
    else if(vision_mode_change_color_flag == BLUE && vision_mode_change_shoot_flag == BUFF)
    {
        vision_cmdid = 0x03;
    }
...
void Gimbal::gimbal_free_control(fp32 *yaw, fp32 *pitch)
{
    ...
    *yaw = yaw_channel * YAW_RC_SEN + gimbal_RC->mouse.x * YAW_MOUSE_SEN;
    *pitch = pitch_channel * PITCH_RC_SEN - gimbal_RC->mouse.y * PITCH_MOUSE_SEN;  // 没改 - 号之前 ， 选手端操作 与 预期相反
}
